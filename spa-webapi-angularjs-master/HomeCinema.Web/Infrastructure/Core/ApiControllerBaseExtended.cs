...
namespace HomeCinema.Web.Infrastructure.Core
{
    public class ApiControllerBaseExtended : ApiController
    {
        ...
        protected IEntityGuidRepository<Profiler> _customersRepository;
        protected IEntityBaseRepository<Image> _imageRepository;
        ...
        private void InitRepositories(List<Type> entities)
        {
            ...
            //if (entities.Any(e => e.FullName == typeof(Profiler).FullName))
            //{
            //    _customersRepository = _dataRepositoryFactory.GetDataRepository<Profiler>(RequestMessage);
            //}

            if (entities.Any(e => e.FullName == typeof(Image).FullName))
            {
                _imageRepository = _dataRepositoryFactory.GetDataRepository<Image>(RequestMessage);
            }
